...
# Traditional-only characters, from
...
_kana = re.compile(r"[\u3040-\u309F\u30A0-\u30FF]")  # Hiragana/Katakana → ja
_hangul = re.compile(r"[\uAC00-\uD7AF\u1100-\u11FF]")  # Hangul → ko
...
def _get_detector() -> LanguageDetector:
    """Lazily built: constructing the detector costs ~15ms and loading its
    models ~35MB, which should not be paid by processes that never detect."""
    global _detector
...
def _reset_detector() -> None:
    global _detector
    _detector = None


register_language_cache_refresh(_reset_detector)


def _zh_variant(s: str) -> str:
    return "zh-tw" if _chn_t.match(s) else "zh-cn"


# A two-or-three word title gives no detector much to work with, and lingua
# spreads its confidence thinly over the candidates. Three cheap corrections
# recover most of the accuracy that costs, all keyed on the observation that
# catalog metadata is overwhelmingly English unless something says otherwise.
#
# English is preferred when it trails the winner by less than this ratio: where
# the true answer is English but lingua picks a neighbour, English still scores
# a sizeable share of the winner's confidence, and where the title genuinely is
# not English, English is usually absent from the ranking altogether.
_EN_NEAR_TIE_RATIO = 0.6
# English is also preferred when the winner itself scores below this, i.e. when
# lingua is effectively guessing between similar languages ("OK Computer" tops
# out at Danish 0.18). Titles opening with an article are exempt: an article is
# real evidence, so a weak winner it agrees with should stand ("Le Havre").
_EN_WEAK_WINNER = 0.2
...
def _detect_latin(s: str) -> str:
    """Language of non-CJK text, with the short-text corrections applied."""
    values = _get_detector().compute_language_confidence_values(s)
    if not values or values[0].value <= 0:
        return "x"
    conf = {v.language.iso_code_639_1.name.lower(): v.value for v in values}
    best = values[0].value
...
def detect_language(s: str, hint: str | None = None) -> str:
    ...
    if not s or not s.strip():
        return "x"
    if _eng.match(s):
        # lingua handles single words far better than langdetect did, but an
    ...
    return _zh_variant(s) if code == "zh" else code
...
def translate(
    ...
    if SiteConfig.system.deepl_api_key:
        ...
        try:
            ...
            r = (j[0] if isinstance(j, list) else j).text
