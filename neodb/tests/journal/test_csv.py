...
class TestCsvExportImport:
    ...
    def test_csv_export_import(self):
                ...
                for filename, expected_data_count in expected_data_rows.items():
                    ...
                    if os.path.exists(file_path):
                        with open(file_path, "r") as file:
                            ...
                            assert row_count == expected_data_count, (
        ...
        importer.run()
        assert importer.message == "11 items imported, 0 skipped, 0 failed."
        ...
        assert mark_book1_imported.visibility == 2
        assert mark_book1_imported.created_time == self.dt
        assert set(mark_book1_imported.tags) == set(
            ["science fiction", "favorite book", "space opera"]
        )

        mark_book2_imported = Mark(self.user2.identity, self.book2)
        assert mark_book2_imported.shelf_type == ShelfType.PROGRESS
        assert mark_book2_imported.comment_text == "Reading it now"
        assert mark_book2_imported.rating_grade is None
        assert set(mark_book2_imported.tags) == set(
            ["science fiction", "desert planet"]
        )
        ...
        assert set(mark_movie1_imported.tags) == set(["mind bender", "science fiction"])
        ...
        assert book1_reviews[0].created_time == self.dt
